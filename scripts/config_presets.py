...
def remove_duplicates(file_path):
    ...
    for line in lines:
        line = line.strip()
        if not line or line not in seen_lines:
    ...
    try:
        ...
        with open(f"{BASEDIR}/{CONFIG_TXT2IMG_CUSTOM_TRACKED_COMPONENTS_FILE_NAME}", "a+") as file:
            ...
            if not existing_data:
                file.writelines(template_data_txt2img)
            else:
                lines_to_add = []
                for line in template_data_txt2img:
                    stripped_line = line.strip()
                    if stripped_line == "" or not any(stripped_line == existing_line.strip() for existing_line in existing_data):
        ...
        remove_duplicates(f"{BASEDIR}/{CONFIG_TXT2IMG_CUSTOM_TRACKED_COMPONENTS_FILE_NAME}")
        remove_extra_newlines(f"{BASEDIR}/{CONFIG_TXT2IMG_CUSTOM_TRACKED_COMPONENTS_FILE_NAME}")

    except FileNotFoundError:
        # config file not found
        # First time running the extension or it was deleted, so fill it with default values
        txt2img_custom_tracked_components_ids = template_data_txt2img

        with open(f"{BASEDIR}/Tracked IDs Templates/config-txt2img-custom-tracked-components.txt", "w") as new_txt2img_tracked_components_file:
            new_txt2img_tracked_components_file.write(''.join(template_data_txt2img))
...
def load_img2img_custom_tracked_component_ids() -> list[str]:
    ...
    try:
        ...
        with open(f"{BASEDIR}/{CONFIG_IMG2IMG_CUSTOM_TRACKED_COMPONENTS_FILE_NAME}", "a+") as file:
            ...
            if not existing_data:
                file.write(''.join(img2img_template_data))
            else:
                lines_to_add = []
                for line in img2img_template_data:
                    stripped_line = line.strip()
                    if stripped_line == "" or not any(stripped_line == existing_line.strip() for existing_line in existing_data):
                        lines_to_add.append(line)
        ...
        remove_duplicates(f"{BASEDIR}/{CONFIG_IMG2IMG_CUSTOM_TRACKED_COMPONENTS_FILE_NAME}")
        remove_extra_newlines(f"{BASEDIR}/{CONFIG_IMG2IMG_CUSTOM_TRACKED_COMPONENTS_FILE_NAME}")

    except FileNotFoundError:
        # config file not found
        # First time running the extension or it was deleted, so fill it with default values
        img2img_custom_tracked_components_ids = img2img_template_data

        with open(f"{BASEDIR}/Tracked IDs Templates/config-img2img-custom-tracked-components.txt", "w") as new_img2img_tracked_components_file:
            new_img2img_tracked_components_file.write(''.join(img2img_template_data))
        print(f"[Config Presets] img2img custom tracked components config file not found, created default config at {BASEDIR}/{CONFIG_IMG2IMG_CUSTOM_TRACKED_COMPONENTS_FILE_NAME}")
